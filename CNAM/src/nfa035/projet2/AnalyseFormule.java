...
public class AnalyseFormule {
	...
	public Contenu formuleToContenu() throws FormuleErroneeException, HorsFeuilleException {
		...
		else if(this.getFormule() ==null || this.getFormule() == "")
			return (Contenu)null;
		else
	...
	private boolean estOperation(String str) {
		...
		if (operateur != null && str.indexOf(operateur.toString()) == str.lastIndexOf(operateur.toString())
				&& operandes.length == 2) {
			if ((estCellule(operandes[0]) || estValeur(operandes[0])) && (estCellule(operandes[1]) || estValeur(operandes[1])))
				return true;
			else
				return false;
		} else
	...
	private Contenu formuleToValeur(String str) throws FormuleErroneeException {
	...
	private Contenu formuleToOperation(String str) throws FormuleErroneeException, HorsFeuilleException {
		...
		if(this.estCellule(operandes[0]))
			operande1= (Contenu)this.getFeuille().getCellule(this.stringToCoordonneeCellule(operandes[0])[0],this.stringToCoordonneeCellule(operandes[0])[1]);
		else if(this.estValeur(operandes[0]))
			operande1 = this.formuleToValeur(operandes[0]);
		else
			throw new FormuleErroneeException();
		if(this.estCellule(operandes[1]))
			operande2= (Contenu)this.getFeuille().getCellule(this.stringToCoordonneeCellule(operandes[1])[0],this.stringToCoordonneeCellule(operandes[1])[1]);
	...
	private Contenu formuleToFonction (String str) throws HorsFeuilleException { 
		str = str.trim().toLowerCase();
		int pointeur;
	...
	private float stringToValeur(String str) throws FormuleErroneeException {
		str = str.trim().toLowerCase();
		if (estValeur(str)) {
			float signe = 1f;
			if('-' == str.charAt(0)) {
